...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::unprepared('CREATE OR REPLACE TRIGGER update_ads_updated_at BEFORE UPDATE ON ads FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();');
        DB::unprepared('CREATE OR REPLACE TRIGGER update_ad_attributes_updated_at BEFORE UPDATE ON ad_attributes FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();');
        DB::unprepared('CREATE OR REPLACE TRIGGER update_ad_images_updated_at BEFORE UPDATE ON ad_images FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();');
        ...
        DB::unprepared('CREATE OR REPLACE FUNCTION update_favorite_count() RETURNS TRIGGER AS $$ BEGIN IF TG_OP = \'INSERT\' THEN UPDATE ads SET favorite_count = favorite_count + 1 WHERE id = NEW.ad_id; RETURN NEW; ELSIF TG_OP = \'DELETE\' THEN UPDATE ads SET favorite_count = favorite_count - 1 WHERE id = OLD.ad_id; RETURN OLD; END IF; RETURN NULL; END; $$ LANGUAGE plpgsql;');
        ...
        DB::unprepared('CREATE OR REPLACE FUNCTION apply_subscription_features() RETURNS TRIGGER AS $$ BEGIN IF NEW.status = \'active\' THEN UPDATE ads l SET priority_score = (SELECT priority_level FROM subscription_plans WHERE id = NEW.plan_id) WHERE l.user_id = NEW.user_id AND l.status = \'active\'; ELSIF NEW.status = \'expired\' OR NEW.status = \'cancelled\' THEN UPDATE ads l SET priority_score = 0 WHERE l.user_id = NEW.user_id AND l.status = \'active\'; END IF; RETURN NEW; END; $$ LANGUAGE plpgsql;');
